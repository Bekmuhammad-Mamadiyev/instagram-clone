...
class User(AbstractUser, BaseModel):
    ...
    def create_verify_code(self, verify_type):
        code = "".join([str(random.randint(1000, 10000) % 10) for _ in range(4)])
...
class UserConfirmation(BaseModel):
    ...
    def save(self, *args, **kwargs):
        if self.verify_type == VIA_EMAIL:  # 30-mart 11-33 + 5minutes
            self.expiration_time = now() + timedelta(minutes=EMAIL_EXPIRE)
            print(self.expiration_time)
        else:
            self.expiration_time = now() + timedelta(minutes=PHONE_EXPIRE)
